...
function FitBounds({ posts }) {
  const map = useMap()
  useEffect(() => {
    const pts = posts.filter(p => p.lat && p.lng)
...
function ContactDisplay({ contact_info }) {
  ...
  if (emailRe.test(contact_info)) {
    return <a href={'mailto:' + contact_info} style={{ color:'#1D4ED8', textDecoration:'none', fontWeight:600 }}>{contact_info}</a>
  }
  if (phoneRe.test((contact_info || '').replace(/^(dad:|mom:|coach:)/i,'').trim())) {
    return <a href={'tel:' + contact_info.replace(/\D/g,'')} style={{ color:'var(--navy)', textDecoration:'none', fontWeight:600 }}>{contact_info}</a>
  }
...
export default function PlayerBoard() {
  ...
  async function handleSubmit() {
    ...
    const payload = {
      post_type: form.post_type, sport: form.sport,
      city: form.city, county: form.region || null,
      ...
      <div className="filter-bar">
        ...
        {user ? (
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <button onClick={() => supabase.auth.signOut()} style={{ ...filterSelectStyle, fontSize:12, padding:'6px 12px', color:'#666' }}>Sign out</button>
          </div>
        ) : (
          <button onClick={() => setShowAuth(true)} style={{ ...filterSelectStyle, fontSize:13, fontWeight:600, color:'var(--navy)' }}>
        ...
        <button onClick={() => {
          ...
          else { setShowForm(true); setEditingId(null); setForm(EMPTY_FORM); setSubmitted(false) }
        }} style={{
          background:'var(--red)', color:'white', border:'none',
          borderRadius:'var(--btn-radius)', padding:'9px 18px',
          fontWeight:700, fontFamily:'var(--font-head)',
          fontSize:14, letterSpacing:'0.04em', textTransform:'uppercase', cursor:'pointer',
        }}>
                  ...
                  <Marker key={post.id} position={[post.lat, post.lng]} icon={makeIcon(color)}>
                    <Popup>
                      <div style={{ fontFamily:'var(--font-body)', minWidth:160 }}>
                        ...
                        {post.age_group && (
                          <div style={{ fontSize:12, marginTop:2 }}>🎯 {post.age_group} · {post.sport}</div>
                        )}
      ...
      {showForm && (
        <div style={{ margin:'24px', background:'white', borderRadius:12, border: isEditing ? '2px solid var(--gold)' : '2px solid var(--lgray)', padding:'24px', maxWidth:680 }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:16 }}>
            {[['player_needed','⚾ Player Needed'],['player_available','🧢 Player Available']].map(([val, label]) => (
              <button key={val} onClick={() => setForm(f => ({
                ...
                team_name:'', age_group:'', position_needed:[],
                venue_name:'', location_address:'', field_number:'',
                event_date:'', distance_travel:25, lat:null, lng:null,
              }))} style={{
          ...
          <div style={{ display:'flex', gap:10, marginTop:8 }}>
            ...
            {isEditing && (
              <button onClick={cancelForm} style={{ background:'white', color:'var(--navy)', border:'2px solid var(--lgray)', borderRadius:8, padding:'12px 20px', fontSize:14, fontWeight:700, cursor:'pointer' }}>
                Cancel
              </button>
            )}
          </div>
        ...
        {filtered.map(post => {
          ...
          return (
            <div key={post.id} style={{
              ...
              {isOwner && (
                <div style={{ position:'absolute', top:-1, right:12, background:'var(--gold)', color:'var(--navy)', fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:'0 0 6px 6px', fontFamily:'var(--font-head)', letterSpacing:'0.04em' }}>
                  YOUR POST
                </div>
              )}
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                ...
                <span style={{ background: post.sport === 'softball' ? '#7C3AED' : '#0B1F3A', color:'white', fontSize:11, fontWeight:700, padding:'3px 9px', borderRadius:20, fontFamily:'var(--font-head)', textTransform:'uppercase' }}>
                  {post.sport}
                </span>
              </div>
