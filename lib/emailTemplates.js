...
export function coachEmail(record, token, duplicates = []) {
  ...
  return { subject: `🧢 New Coach Submission — ${record.name}`,
...
export function teamEmail(record, token, duplicates = []) {
  ...
  return { subject: `⚾ New Team Submission — ${record.name}`,
...
export function playerBoardEmail(record, token, duplicates = []) {
  ...
  const rows = [
    ...
    ...(isPlayerNeeded ? [row('Team Name', record.team_name), row('Age Group', record.age_group),
      row('Position(s) Needed', record.position_needed), row('Event / Location', record.location_name),
      row('Event Date', record.event_date)] : []),
    ...(!isPlayerNeeded ? [row('Player Age', record.player_age), row('Age Group', record.age_group),
      row('Position(s)', record.player_position), row('Team Name', record.team_name),
  ...
  return { subject: `📋 New Player Board Post — ${isPlayerNeeded ? 'Player Needed' : 'Player Available'}`,
...
export function facilityEmail(record, token, duplicates = []) {
  ...
  return { subject: `🏟️ New Facility Submission — ${record.name}`,
