...
function wrapper(title, badgeColor, badgeLabel, tableRows, buttons, duplicates = [], introText = 'A new submission is waiting for your review. Click Approve or Reject below.') {
...
export function coachEmail(record, token, duplicates = []) {
  const rows = [
    ...
    row('Latitude', record.lat != null ? String(record.lat) : 'Not resolved'),
    row('Longitude', record.lng != null ? String(record.lng) : 'Not resolved'),
    row('Geocode Source', record.geocode_source || 'Not resolved'),
  ].join('');
  return {
...
export function reviewEmail(record, token) {
  const coachLabel = record.coach_name || 'Unknown coach';
  const rows = [
    row('Coach', coachLabel),
    row('Review ID', record.id),
  ...
  const buttons =
    record.id && token
      ? approvalButtons('reviews', record.id, token)
      : '<p style="margin:24px 0 0;font-size:13px;color:#6b7280;">Review submitted by a site visitor. Moderate this review in Supabase.</p>';

  return {
    subject: `⭐ New Coach Review — ${coachLabel}`,
    html: wrapper(`New Coach Review: ${coachLabel}`, '#f59e0b', 'Coach Review', rows, buttons),
  };
}

export function advertiserInquiryEmail(record) {
  const rows = [
    row('Business Name', record.business_name),
    row('Contact Name', record.contact_name),
    row('Email', record.email),
    row('Phone', record.phone),
    row('Website', record.website_url),
    row('Business City / State', record.business_city_state),
    row('Placement Interest', record.placement_interest),
    row('Target Geography', record.target_geography),
    row('Business Description', record.business_description),
    row('Notes / Goals', record.notes_goals),
    row('Status', record.status || 'new'),
    row('Source', record.source || 'advertise_page'),
    row('Submitted At', record.created_at),
  ].join('');

  const noButtons = '<p style="margin:24px 0 0;font-size:13px;color:#6b7280;">Follow up manually from your inbox, then update the inquiry record status in Supabase as needed.</p>';

  return {
    subject: `📣 New Advertiser Inquiry — ${record.business_name || 'Unknown Business'}`,
    html: wrapper(
      `New Advertiser Inquiry: ${record.business_name || 'Unknown Business'}`,
      '#0f766e',
      'Advertiser Inquiry',
      rows,
      noButtons,
      [],
      'A new advertiser inquiry has been submitted through the public Advertise page.'
    ),
  };
}